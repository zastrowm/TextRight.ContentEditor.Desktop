    ...
    public void VerifyItWorks()
    {
      var it = DoAll(
        new Func<IUndoableAction>[]
        {
          ...
          () => new DeletePreviousCharacterCommand.UndableAction(BlockAt(0).BeginCursor(3).AsTextCursor()),
        });
      ...
      for (int i = 1; i < text.Length + 1; i++)
      {
        var it = DoAll(
          new Func<IUndoableAction>[]
          {
            ...
            () => new DeletePreviousCharacterCommand.UndableAction(BlockAt(0).BeginCursor(i).AsTextCursor()),
          });
