...
namespace TextRight.Core.Editing
{
  ...
  /// </remarks>
  internal abstract class BlockCaretMover
  {
    private const bool DidNotMove = false;

    private const bool DidMove = true;

    public abstract BlockCaret MoveTowards(BlockCaret cursor);

    public abstract BlockCaret MoveAway(BlockCaret cursor);

    public abstract bool DidReachEdge(BlockCaret cursor);

    /// <summary> A cursor mover whose direction is forward by default. </summary>
    public static readonly BlockCaretMover ForwardMover
      = new MoveForwardMover();

    /// <summary> A cursor mover whose direction is backward by default. </summary>
    public static readonly BlockCaretMover BackwardMover
      = new MoveBackwardMover();

    public BlockCaret MoveCaretTowardsLineEdge(BlockCaret cursor)
    {
    ...
    public BlockCaret MoveTowardsLine(BlockCaret cursor, double y)
    {
      //using (var closestLine = CursorSnapshot.From(cursor))

      var closestLine = cursor;

      var lastPosition = cursor.Measure();

      while (TryMoveTowards(ref cursor))
      {
        var newPosition = cursor.Measure();
    ...
    public bool TryMoveTowards(ref BlockCaret caret)
    {
      BlockCaret maybe = MoveTowards(caret);
      if (maybe.IsValid)
      {
        caret = maybe;
        return true;
    ...
    /// <summary> A cursor mover whose direction is backward by default. </summary>
    private class MoveBackwardMover : BlockCaretMover
    {
      public override BlockCaret MoveTowards(BlockCaret cursor)
        => cursor.MoveBackward();

      public override BlockCaret MoveAway(BlockCaret cursor)
        => cursor.MoveForward();

      public override bool DidReachEdge(BlockCaret cursor)
        => cursor.IsAtBlockStart;
    }

    /// <summary> A cursor mover whose direction is forward by default. </summary>
    private class MoveForwardMover : BlockCaretMover
    {
      public override BlockCaret MoveTowards(BlockCaret cursor)
        => cursor.MoveForward();
